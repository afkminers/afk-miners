...
// domínio do cookie (prod: normalmente .afkminers.com)
...
// secure dos cookies (https only em produção)
...
// se SameSite=None, o navegador exige secure=true
if (COOKIE_SAME_SITE === 'None' && !EFFECTIVE_SECURE) {
  EFFECTIVE_SECURE = true;
}

// modo debug de CSRF (liga com CSRF_DEBUG=1 no .env)
const CSRF_DEBUG = String(process.env.CSRF_DEBUG || '0') === '1';
...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    console.error('[auth] erro ao buscar usuário da sessão:', err);
...
// gera cookie + token para o client
function csrfRoute(req, res) {
  const t = randomBytes(24).toString('hex');

  const host = String(req.hostname || '').toLowerCase();
  // em localhost NUNCA aplicamos domain, pra evitar cookie não setar
  const useDomain =
    COOKIE_DOMAIN &&
    host &&
    host !== 'localhost' &&
  ...
  res.cookie(CSRF_COOKIE, t, {
    httpOnly: false, // client precisa ler e mandar de volta
    sameSite: COOKIE_SAME_SITE,
    secure: EFFECTIVE_SECURE,
    path: '/',
...
function requireCsrf(req, res, next) {
  ...
  if (!originOk || !refererOk) {
    if (CSRF_DEBUG) {
      console.warn('[CSRF] blocked by origin', { origin, referer, originOk, refererOk });
    }
    logLeaderboardBlock(req, 'csrf-bad-origin');
    return res.status(403).json({ error: 'Bad origin' });
  }

  // 2) token (double submit)
  const headerToken = (
  ...
  if (CSRF_DEBUG) {
    console.log('[CSRF] token check', {
      path: req.path,
      method: m,
      cookie: shortToken(ck),
      header: shortToken(headerToken),
      body: shortToken(bodyToken),
      query: shortToken(queryToken),
      candidates: candidates.map(shortToken),
      valid,
    });
  }
