...
// - (opcional) docs/context/symbol-index.json [via --symbols ou CTX_SYMBOLS=1]
// - (opcional) docs/context/deps.txt          [via --imports ou CTX_IMPORTS=1]
// Execução típica: node scripts/gen-context.js --full
//   Flags: --full (equivale a --symbols --imports --depth 4)
//          --symbols
//          --imports
//          --depth N
...
// Se --full, habilita tudo e depth default 4 (a menos que --depth N seja passado)
if (hasFlag('--full')) {
  process.env.CTX_SYMBOLS = process.env.CTX_SYMBOLS || '1';
  process.env.CTX_IMPORTS = process.env.CTX_IMPORTS || '1';
  process.env.CTX_DEPTH = process.env.CTX_DEPTH || '4';
}
if (hasFlag('--symbols')) process.env.CTX_SYMBOLS = '1';
if (hasFlag('--imports')) process.env.CTX_IMPORTS = '1';
const depthArg = getFlagValue('--depth');
if (depthArg) process.env.CTX_DEPTH = String(depthArg);

// -------------------- Constantes --------------------
const SCRIPT_DIR = __dirname;                      // .../server/scripts
...
// -------------------- Git info --------------------
function gitInfo(){
  const hash = sh('git rev-parse --short HEAD') || 'n/a';
  const branch = sh('git rev-parse --abbrev-ref HEAD') || 'n/a';
  // --always evita "fatal" quando não há tags
  const lastTag = sh('git describe --tags --abbrev=0 --always') || '';
...
function largestFiles(n = SHOW_TOP_LARGEST){
  ...
  const sizes = [];
  for(const f of all){
    try { const st = fs.statSync(f); sizes.push({f, b: st.size}); } catch {}
  }
...
function buildSymbolIndex(){
  ...
  for(const f of files){
    ...
    let r; while((r = rxReExport.exec(src)) !== null){
...
function buildImportGraph(){
  const files = glob.sync('**/*.{js,ts,jsx,tsx}', { nodir:true, ignore: GLOB_IGNORE });
  const edges = [];
  const rxImport1 = /^\s*import\s+.*?\s+from\s+['"]([^'"]+)['"]/m;
  const rxImport2 = /^\s*import\s+['"]([^'"]+)['"]/m;
  for(const f of files){
    let src=''; try{ src = fs.readFileSync(f,'utf8'); }catch{ continue; }
    const lines = src.split(/\r?\n/);
    for(const line of lines){
...
function genCtxYmlIfMissing(info){
...
function main(){
  ...
  if(process.env.CTX_IMPORTS === '1'){
    ...
    const sample = (Array.isArray(deps)? deps : []).slice(0,50).map(d=> `${d.from} -> ${d.to}`);
