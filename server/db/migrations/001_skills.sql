...
CREATE TABLE IF NOT EXISTS skill_curves (
  ...
  level        INTEGER     NOT NULL,     -- nível alvo (ex.: precisa de X tries para ir do 14→15)
  tries_needed INTEGER     NOT NULL,     -- número de tries requeridos
...
CREATE TABLE IF NOT EXISTS class_skill_rates (
  class       VARCHAR(32) NOT NULL,      -- ex.: 'warrior','ranger','mage','guardian',...
  skill_type  VARCHAR(24) NOT NULL,
  rate        REAL        NOT NULL DEFAULT 1.0,  -- >1 facilita treinar; <1 dificulta
  PRIMARY KEY (class, skill_type)
);

-- 3) Mapa de tipo de ARMA → skill treinada
CREATE TABLE IF NOT EXISTS weapon_skill_map (
  weapon_type VARCHAR(32) PRIMARY KEY,   -- ex.: 'sword','axe','daggers','bow','mace_shield',...
...
CREATE TABLE IF NOT EXISTS player_hero_skills (
  ...
  skill_type  VARCHAR(24) NOT NULL,
  level       INTEGER     NOT NULL DEFAULT 10,   -- ponto de partida (ajustável)
  tries       INTEGER     NOT NULL DEFAULT 0,    -- tries acumulados no nível atual
  need        INTEGER     NOT NULL,              -- tries_needed do nível atual (cacheado p/ performance)
