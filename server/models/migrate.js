...
async function migrate() {
  ...
  // ---------- Loot phase 1 (corpses + ground items) ----------
  await run(`
  ...
  `);
  await run('CREATE INDEX IF NOT EXISTS idx_monster_corpses_map ON monster_corpses(map_key)');
  await run('CREATE INDEX IF NOT EXISTS idx_monster_corpses_owner ON monster_corpses(owner_player_id)');
  await run('CREATE INDEX IF NOT EXISTS idx_monster_corpses_expires_at ON monster_corpses(expires_at)');
  ...
  `);
  await run('CREATE INDEX IF NOT EXISTS idx_corpse_items_corpse ON monster_corpse_items(corpse_id)');
  ...
  `);
  await run('CREATE INDEX IF NOT EXISTS idx_ground_items_map ON ground_items(map_key)');
  await run('CREATE INDEX IF NOT EXISTS idx_ground_items_expires ON ground_items(expires_at)');
