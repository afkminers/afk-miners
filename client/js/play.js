...
function initTopbarActions() {
  ...
  if (menu) {
    ...
    const dropdown = menu.querySelector('[data-menu-content]');
    const openMenu = () => {
...
function normalizeHeroPayload(raw = {}) {
  ...
  const existing = profileState.heroesById.get(id) || null;
  ...
  if (!(xpNeeded > 0) && existing && Number.isFinite(existing.xpNeeded) && existing.xpNeeded > 0) {
    xpNeeded = existing.xpNeeded;
  }
...
function getPartyListElement() {
  return document.querySelector('[data-party-list]');
}

function resolvePartyHeroIds() {
  if (window.Team?.getActiveTeamIds) {
    const ids = window.Team.getActiveTeamIds();
    if (Array.isArray(ids) && ids.length) return ids.map((id) => String(id));
  }
  if (window.ActiveHeroId) return [String(window.ActiveHeroId)];
  if (profileState.heroes.length > 0) {
    return profileState.heroes.map((hero) => hero.id).filter(Boolean).slice(0, 3);
  }
  return [];
}

function renderParty() {
  const listEl = getPartyListElement();
  if (!listEl) return;
  const heroIds = resolvePartyHeroIds();
  listEl.innerHTML = '';
  if (!heroIds.length) {
    const empty = document.createElement('li');
    empty.className = 'party-empty';
    empty.textContent = 'Sem heróis ativos';
    listEl.appendChild(empty);
    return;
  }
  heroIds.forEach((heroId) => {
    const hero = getHeroFromState(heroId);
    if (!hero) return;
    const item = document.createElement('li');
    item.className = 'party-member';
    const name = hero.name ? escapeHtml(hero.name) : `Herói ${heroId}`;
    const hpText = `${numberFormatter.format(Math.max(0, clampNumber(hero.hp, 0)))} / ${numberFormatter.format(Math.max(0, clampNumber(hero.maxHp, 0)))}`;
    const mpCurrent = clampNumber(hero.mana ?? hero.mp ?? 0, 0);
    const mpMax = clampNumber(hero.maxMana ?? hero.mpMax ?? hero.manaMax ?? mpCurrent, mpCurrent);
    const mpText = `${numberFormatter.format(mpCurrent)} / ${numberFormatter.format(mpMax)}`;
    item.innerHTML = `
      <span class="name">${name}</span>
      <span class="hp">HP ${hpText}</span>
      <span class="mp">MP ${mpText}</span>
    `;
...
function setupSkillAdvanceBridge(attempt = 0) {
  if (skillAdvanceHooked) return;
  const adv = window.SkillAdvance;
  if (!adv || !adv.monitor || typeof adv.monitor.update !== 'function') {
    if (attempt < 20) {
      setTimeout(() => setupSkillAdvanceBridge(attempt + 1), 500);
    }
    return;
  }
  const originalUpdate = adv.monitor.update.bind(adv.monitor);
  adv.monitor.update = function patchedSkillUpdate(heroId, heroName, rows) {
    const result = originalUpdate(heroId, heroName, rows);
    try {
      if (Array.isArray(rows)) {
        const key = heroId != null ? String(heroId) : null;
        if (key) {
          skillsCache.set(key, rows);
          if (window.ActiveHeroId && String(window.ActiveHeroId) === key) {
            renderSkills(rows);
          }
        }
      }
    } catch (err) {
      console.warn('[play] skill monitor update patch failed:', err?.message || err);
    }
    return result;
  };
  skillAdvanceHooked = true;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startHudTimers();
  initTopbarActions();
  setupSkillAdvanceBridge();
  try {
    const maybePromise = initFriendHud();
    if (maybePromise && typeof maybePromise.catch === 'function') {
      maybePromise.catch((err) => {
        console.warn('[play] initFriendHud failed:', err?.message || err);
      });
    }
  } catch (err) {
    console.warn('[play] initFriendHud failed:', err?.message || err);
